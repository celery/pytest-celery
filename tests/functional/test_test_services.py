...
def test_test_service_basic_functionality(test_service_cls, subtests, faker):
    test_service = test_service_cls(faker.uuid4())
    ...
    with subtests.test("Ensure docker container is running", test_service_cls=test_service_cls):
        containers = client.containers.list(filters={"id": docker_container_id})
        assert len(containers) == 1

    with subtests.test("Ensure test service is responsive", test_service_cls=test_service_cls):
        test_service.ping()

    test_service.stop()

    with subtests.test("Ensure container is now stopped", test_service_cls=test_service_cls):
        containers = client.containers.list(filters={"id": docker_container_id})
