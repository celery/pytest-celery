...
def test_message_broker_basic_functionality(message_broker_cls, subtests, faker):
    ...
    with subtests.test(
        "Ensure docker container is running", message_broker_cls=message_broker_cls
    ):
    ...
    with subtests.test(
        "Ensure message broker is responsive", message_broker_cls=message_broker_cls
    ):
        message_broker.ping()
    ...
    with subtests.test(
        "Ensure container is now stopped", message_broker_cls=message_broker_cls
    ):
        containers = client.containers.list()
